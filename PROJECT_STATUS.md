...
| WASM/browser fallback: builds, Node-verified | `web_demo/{ulunas_wasm.cpp,.wasm,.js}`, `test_wasm_node.js` | PASS | Identity-passthrough only (no model wired in yet — same reason as the iOS bridge); explicitly labeled as not proving native ARM64/NEON performance |
