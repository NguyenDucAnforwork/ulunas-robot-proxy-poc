"""
Direct comparison: F_PROXY_ROBOT (lamda_mag=70, original) vs F_PROXY_ROBOT_LOWSUP
(lamda_mag=35, single-variable change) on the same robot-proxy synthetic test set (563
utterances) used in eval_robot_proxy.py, with paired bootstrap CI, overall and on the
UAV-motor_high + procedural-fan "tradeoff subset" from QC_FAILURE_ANALYSIS.md §4.2.
"""
import json
import os
import sys
...
CONDITIONS = ["N0", "F_PROXY_ROBOT", "F_PROXY_ROBOT_LOWSUP"]
...
def main():
    ...
    # paired bootstrap CI: F_PROXY_ROBOT_LOWSUP vs F_PROXY_ROBOT, overall + tradeoff subset
    def ci_for(metric, idx=None):
        a = per_cond["F_PROXY_ROBOT_LOWSUP"][metric]
    ...
    with open(f"{OUT_DIR}/lowsup_comparison_results.json", "w") as f:
