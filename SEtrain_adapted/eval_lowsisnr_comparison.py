"""
Direct comparison: F_PROXY_ROBOT (lamda_sisnr=1.0, original) vs F_PROXY_ROBOT_LOWSISNR
(lamda_sisnr=0.3, lamda_ri/lamda_mag back to original 30/70 -- single-variable change vs
F_PROXY_ROBOT, testing whether the SI-SNR term's relative gradient weight -- not lamda_mag,
per the failed §4.3c ablation -- drives the SIG-BAK over-suppression tradeoff) on the same
robot-proxy synthetic test set (563 utterances) used in eval_robot_proxy.py, with paired
bootstrap CI, overall and on the UAV-motor_high + procedural-fan "tradeoff subset" from
QC_FAILURE_ANALYSIS.md §4.2.
"""
import json
import os
import sys
...
CONDITIONS = ["N0", "F_PROXY_ROBOT", "F_PROXY_ROBOT_LOWSISNR"]
...
def main():
    ...
    # paired bootstrap CI: F_PROXY_ROBOT_LOWSISNR vs F_PROXY_ROBOT, overall + tradeoff subset
    def ci_for(metric, idx=None):
        a = per_cond["F_PROXY_ROBOT_LOWSISNR"][metric]
        b = per_cond["F_PROXY_ROBOT"][metric]
        if idx is not None:
            a = [a[i] for i in idx]; b = [b[i] for i in idx]
        delta, lo, hi = paired_bootstrap_delta_ci(a, b)
        return {"delta": delta, "ci95_lo": lo, "ci95_hi": hi}

    results["ci_lowsup_vs_original_overall"] = {
        m: ci_for(m) for m in ["sig", "bak", "ovrl", "sisdr", "stoi", "per_utt_wer"]
    }
    results["ci_lowsup_vs_original_tradeoff_subset"] = {
        m: ci_for(m, idx_tradeoff) for m in ["sig", "bak", "ovrl", "sisdr", "stoi", "per_utt_wer"]
    }

    with open(f"{OUT_DIR}/lowsisnr_comparison_results.json", "w") as f:
